...
{
    fprintf (stdout, "%s:%d data <%s>. data is searcher's inparam. it can be compare to id's subelement. \n",__func__,__LINE__,
    (char*)data);
...
main (int argc, char **argv)
{
...
#ifdef __CASE__
	 fprintf (stderr,
        "Usage: acsmx pattern word-1 word-2 ... word-n  \n");
#else
	 fprintf (stderr,
  ...
  acsmCompile (acsm);
  acsmSearch (acsm, text, strlen (text), MatchFound,  (void *) text, &state);
  acsmFree (acsm);
